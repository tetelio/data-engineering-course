...
# Prepare directory variables and ensure that the directory for our decrypted assets exists
...
# Read the key string from the .env file to encrypt the files
...
# Transform the key string as a byte array in utf-8 (which converts the bytes into a numeric value between 0 and 255) and calculate its length in order to decrypt
...
for file_path in encrypted_assets_path.iterdir():
    ...
    # Read the bytes of the original file 
    original_file_path = assets_path / str(file_path.name).replace('_encrypted', '')

    with open(original_file_path, 'rb') as f:
        original_file_bytes = bytearray(f.read())

    # Compare the original bytes with the decrypted bytes. If they are not the same, something went wrong so file will not be saved to disk 
